...
void setup()
{
  ...
  delay(500);
	...
	// You can use up to 16 timer for each ESP32_ISR_Timer
	for (uint16_t i = 0; i < NUMBER_ISR_TIMERS; i++)
	{
#if USE_COMPLEX_STRUCT
		curISRTimerData[i].previousMillis = startMillis;
		ESP32_ISR_Timer.setInterval(curISRTimerData[i].TimerInterval, curISRTimerData[i].irqCallbackFunc);
#else
		previousMillis[i] = millis();
		ESP32_ISR_Timer.setInterval(TimerInterval[i], irqCallbackFunc[i]);
#endif
	}
...
void loop()
{
	...
	// You don't need to and never call ESP32_ISR_Timer.run() here in the loop(). It's already handled by ISR timer.
	simpleTimer.run();
}
